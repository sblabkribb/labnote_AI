...
LLAMA_CPP_PATH C:/Users/user/Desktop/llama.cpp"
...
GGUF_FILE_PATH="${GGUF_OUTPUT_DIR}/${NEW_OLLAMA_MODEL_NAME}_${QUANTIZE_METHOD}.gguf"

# FP16 GGUF로 변환
python3 "${LLAMA_CPP_PATH}/convert.py" "${TRAINED_MODEL_DIR}" \
  --outfile "${GGUF_FILE_PATH}" \
  --outtype f16

# 지정된 방식으로 양자화
"${LLAMA_CPP_PATH}/quantize" "${GGUF_FILE_PATH}" "${QUANTIZE_METHOD}"
