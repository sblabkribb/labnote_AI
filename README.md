...
## ✨ 주요 기능
  ...
  * **중앙 집중형 AI 모델**: 모든 사용자의 피드백을 중앙 서버에 모아 **하나의 강력한 모델을 공동으로 학습**시켜, 쓸수록 똑똑해지는 집단 지성을 구현합니다.
  * **사용자 수정 기반 DPO 학습**: AI의 제안을 사용자가 직접 수정한 **최종 결과물**을 학습하여, 연구원의 실제 의도와 스타일에 더 가깝게 결과물을 생성합니다.
  * **통합된 수정 및 학습 UI**: AI가 제안한 내용을 Webview 내에서 **선택하고 바로 수정**한 뒤, '적용' 버튼 하나로 연구노트 반영과 AI 학습을 동시에 수행합니다.
  * **사용자 동의 절차**: AI 학습을 위한 데이터 활용에 대해 **최초 실행 시 명시적인 동의**를 얻어, 투명하고 신뢰도 높은 시스템을 운영합니다.
...
LabNote AI Assistant는 사용자와의 상호작용을 통해 발전하는 AI 모델을 핵심으로 합니다. AI 성능 개선을 위해 일부 데이터가 수집될 수 있으며, 이에 대한 투명한 정책은 다음과 같습니다.

### 1. 데이터 수집 및 활용 동의

-   **최초 동의**: `LabNote (AI): 섹션 내용 채우기`와 같이 AI 학습과 관련된 기능을 **최초로 실행**할 때, 데이터 수집 및 활용에 대한 동의를 구하는 안내창이 표시됩니다.
-   **동의 필수**: 이 기능들을 사용하기 위해서는 약관에 **동의**해야 합니다. 동의하지 않을 경우, 해당 기능은 비활성화됩니다.
-   **동의 상태 변경**: VS Code 설정에서 `labnote.ai.consentStatus` 값을 변경하여 언제든지 동의 상태를 철회하거나 다시 동의할 수 있습니다.

### 2. 수집하는 정보

AI 모델의 성능 개선을 위해 수집되는 정보는 다음과 같으며, 모든 정보는 **사용자를 식별할 수 없는 익명 상태**로 처리됩니다.

-   **사용자 프롬프트**: '섹션 내용 채우기' 실행 시 AI에게 전달되는 문맥 정보 (예: 실험 목표, 유닛 오퍼레이션 ID 및 이름, Input/Output 섹션 내용).
-   **AI 제안**: AI가 생성한 여러 버전의 텍스트 초안.
-   **사용자 피드백**:
    -   사용자가 선택한 AI 제안 원본 (chosen\_original)
    -   사용자가 최종적으로 수정한 텍스트 (chosen\_edited)
    -   사용자가 선택하지 않은 나머지 AI 제안 (rejected)

### 3. 사용 목적

수집된 모든 데이터는 오직 **LabNote AI의 언어 모델(LLM) 성능을 개선하고, 사용자의 의도에 더 부합하는 정확하고 유용한 텍스트를 생성하기 위한 DPO(Direct Preference Optimization) 학습 목적**으로만 사용됩니다. 데이터는 외부 제3자에게 판매되거나 공유되지 않습니다.

### 4. 저작권 및 책임

-   **저작권 귀속**: 사용자가 AI의 도움을 받아 작성하고 최종 수정한 연구노트의 모든 콘텐츠에 대한 저작권은 **사용자 또는 사용자의 소속 기관**에 있습니다.
-   **최종 책임**: AI가 생성한 내용은 초안이며, 그 내용의 정확성, 완전성, 적절성에 대한 검증 및 최종 책임은 **사용자 본인**에게 있습니다.
